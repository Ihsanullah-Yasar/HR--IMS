"use client";
import { Button } from "@/components/ui/button";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import Link from "next/link";
import { Card, CardHeader, CardTitle, CardContent } from "@/components/ui/card";
import { Plus } from "lucide-react";
import { DataTable } from "@/components/ui/advance-table";
import getCurrencyColumns from "../columns/currency-column";
import { useState } from "react";
import { ApiResponse, Meta } from "@/lib/Types/api";
import { toast } from "sonner";
import { useSearchParams } from "next/navigation";
import { DeleteAlertDialog } from "@/components/ui/delete-alert-dialog";
import { TableLoadingSkeleton } from "../shared/skeleton/tableSkeleton";
import { Currency } from "@/lib/Types/currency";
import {
  deleteCurrency,
  getCurrencies,
} from "@/lib/Actions/currencies/currencies";
import {
  currencyFilterConfig,
  currencySortOptions,
} from "../filterConfigs/currencies";
import DynamicTableFilters from "../table-filters";

export default function CurrencyTable() {
  const queryClient = useQueryClient();
  const searchParams = useSearchParams();
  const paginationData = { page: 1, perPage: 10 };

  // Delete dialog state
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [currencyToDelete, setCurrencyToDelete] = useState<Currency | null>(
    null
  );

  const { data, isLoading, error, isError, refetch } = useQuery({
    queryKey: [
      "currencies",
      paginationData.page,
      paginationData.perPage,
      searchParams.toString(),
    ],
    queryFn: () => getCurrencies(searchParams.toString()),
    select: (response) => response as ApiResponse<Currency[]>,
  });

  const { mutateAsync: deleteCurrencyMutation } = useMutation({
    mutationFn: deleteCurrency,
    onSuccess: () => {
      toast.success("Currency deleted successfully");
      queryClient.invalidateQueries({ queryKey: ["currencies"] });
    },
  });

  if (isLoading) {
    return <TableLoadingSkeleton columns={7} rows={10} />;
  }

  if (isError) {
    return (
      <div className="flex flex-col items-center align-center py-8">
        <p className="text-red-500">{error.message}</p>
        <Button onClick={() => refetch()} size="sm" className="mt-2">
          Retry
        </Button>
      </div>
    );
  }

  const handleDeleteClick = (currency: Currency) => {
    setCurrencyToDelete(currency);
    setDeleteDialogOpen(true);
  };

  const handleDeleteConfirm = async () => {
    if (currencyToDelete) {
      await deleteCurrencyMutation(currencyToDelete.id);
      setCurrencyToDelete(null);
    }
  };

  return (
    <>
      <Card>
        <CardHeader className="px-6 py-4">
          <div className="flex justify-between items-center w-full">
            <CardTitle className="text-xl font-semibold">Currencies</CardTitle>
            <Link href="/dashboard/currencies/create">
              <Button size="sm" className="h-8">
                <Plus className="h-4 w-4 mr-1" /> Add Currency
              </Button>
            </Link>
          </div>
        </CardHeader>
        <CardContent className="px-6">
          <div className="space-y-4">
            <DynamicTableFilters
              title="Currency Filters"
              sortOptions={currencySortOptions}
              filterConfigs={currencyFilterConfig}
            />

            <div className="rounded-md border overflow-hidden">
              <DataTable
                columns={getCurrencyColumns({
                  onDeleteClick: handleDeleteClick,
                })}
                data={data?.data ? data?.data : ([] as Currency[])}
                meta={data?.meta ? data.meta : ({} as Meta)}
              />
            </div>
          </div>
        </CardContent>
      </Card>

      {/* Single DeleteAlertDialog at the page level */}
      <DeleteAlertDialog
        open={deleteDialogOpen}
        onOpenChange={setDeleteDialogOpen}
        title="Are you absolutely sure?"
        description="This action cannot be undone. This will permanently delete"
        itemName={currencyToDelete ? `currency ${currencyToDelete.code}` : ""}
        onConfirm={handleDeleteConfirm}
      />
    </>
  );
}
